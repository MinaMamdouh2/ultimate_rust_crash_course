...
fn main() {
    ...
    let number = convert_to_integer(number); // uncomment this line and finish shadowing `number`
    ...
    let sum = add(answer, number); // call your `add` function and pass it `number` and `answer` as arguments.
